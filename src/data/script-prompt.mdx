...
The first step in generating a scene is to generate a script. Through prompt engineering, we developed a modular prompting system that combines the descriptions of masks, props and backdrops, as well as constrains around the desired output. We then sent the prompt to a system of models downloaded and running locally on the [Leonardo Supercomputer](#content-hardware-and-batching). 

The first model is the 72billion parameter, instruction-tuned version of Qwen 2.5 and we run the prompting via Python using the Hugging Face [transformers](https://huggingface.co/docs/transformers/main/en/index) library which allows us to pass both a system prompt and a user prompt.
...
The user prompt is constructed from a modular template that takes each element of [the scene](#content-a-scene) (masks, prop, setting) and wraps it in instructional language to trigger the LLM to produce a script-like output rather than something else. It also further adds control to the way these bits of information may return or be used in the script:
