...
The first step in generating a scene is to generate a script. Through <SidebarLink content={<SectionPromptTactics />}>prompt engineering</SidebarLink>, we developed a modular prompting system that combines the descriptions of masks, props and backdrops, as well as constrains around the desired output. We then sent the prompt to a system of models downloaded and running locally on the [Leonardo Supercomputer](#content-hardware-and-batching). 
