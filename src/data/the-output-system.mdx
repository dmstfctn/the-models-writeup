...
A central BaseSystem governs simulation states (including Construct, PlayBack, Conclude), coordinates subsystems, syncs with a server, and communicates with audience phones. A PlaybackSystem loads AI-generated scripts, guides <SidebarLink content={<SectionTheDirector/>}>the director’s</SidebarLink> setup, and feeds masks lines during each scene, controlling the movement of each masks’ multiple mouths. Different actions - turn, tumble and spin - are potentially triggered when a mask speaks a line.