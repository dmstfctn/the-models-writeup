...
<Fig
    ...
    width={1909}
    height={889}
    caption={`Most elements visible on stage, such as theatrical silhouettes, masks and props are drawn by hand, then digitised, 3D modelled and collected inside Unreal Engine.`}
/>
...
There are on-stage and back-stage areas defined by collision volumes, with masks set up to behave differently depending on whether they’re visible, and target points for different stage positions. Rather than spawning items on stage, or having set animations, each scene is constructed by <SidebarLink content={<SectionTheDirector/>}>the director</SidebarLink> moving between regions, picking up and placing objects and masks on stage. This means timing of actions is not definitive, but influenced by distance and collision between elements. 
...
A central BaseSystem governs simulation states (including Construct, PlayBack, Conclude), coordinates subsystems, syncs with [a server](#content-interface-and-server), and communicates with [audience phones](#content-interface-and-server). A PlaybackSystem loads [AI-generated scripts](#content-script-prompt), guides <SidebarLink content={<SectionTheDirector/>}>the director’s</SidebarLink> setup, and feeds [masks](#content-a-scene) lines during each scene, controlling the movement of each masks’ multiple mouths. Different actions - turn, tumble and spin - are potentially triggered when a mask speaks a line.